...
## Issue #37: 2段階レンダリング（低解像度プレビュー→フル解像度差し替え）

### 背景

#34（キャッシュ・プリフェッチ）と #36（RenderQueue による優先度制御）で、アプリ側の無駄
（全ページ待ってから開く、キャッシュ無制限、先読みなし、worker の順番をアプリ側で制御できない）
は解消済み。それでも残る待ちは、pdf.js が重い1ページ（高解像度スキャン画像など）を
ラスタライズする素の時間そのもので、キューやキャッシュではこの時間自体は縮まらない。
止まったページが画面に出るまで「白いまま」の一拍が残る。

### 方式

新規優先度と、フル解像度が出来上がるまでのつなぎとして低解像度プレビューを挟む2段階
レンダリングを導入した。

- `RenderQueue`（`src/viewer/render-queue.ts`）に最上位の優先度 `"preview"` を追加し、
  ランクを `preview > visible > overscan > prefetch` に振り直した。画面に入ったばかりの
  ページの「ぼやけた即時表示」は、他ページのフル描画（`"visible"` 含む）を追い越して
  最速で worker に届く必要があるため
- `previewScale`（`src/pdf/canvas-scale.ts`）: プレビューの backing store ピクセル数が
  `PREVIEW_MAX_PIXELS`（≈1MP）に収まるスケールを返す純関数。固定比（例: 1/4）ではなく
  ピクセル数上限で丸めているのは、高ズーム時にフル解像度と一緒にプレビューまで肥大化するのを
  防ぐため（固定比だと「軽いはずのプレビュー」が意味を失う）。フル描画自体の backing store が
  最初から `maxPixels` 以下（低ズーム・小さいページなど）なら、プレビューを挟む意味がないため
  `null` を返し、呼び出し側は従来どおり1段階描画にフォールバックする
- `PageCanvas`（`src/viewer/PageCanvas.tsx`）: キャッシュミスかつ `priority === "visible"`
  かつ `queue` が渡されている（＝サムネイルではない）ときだけ2段階になる。overscan/prefetch
  のページで先にプレビューを出しても、それは画面外であり、`"preview"` の最上位ランクで
  他ページの visible フル描画を割り込んで邪魔するだけなので対象外とした
  - プレビュー用 canvas を即マウントし（従来の空 canvas と同じ位置づけ）、`"preview"`
    優先度で低解像度描画。`renderPage` は自分の CSS サイズを描画スケールに合わせて
    設定してしまうため、完了後に `style.width`/`style.height` を props の表示サイズへ
    上書きして引き伸ばす
  - フル解像度描画は別の canvas（**DOM 外**）に対して行い、完了して初めて
    `host.replaceChildren(fullCanvas)` で原子的に差し替える。差し替えの瞬間まで
    プレビュー canvas を残しておくことで、フル解像度が出来上がるまでの間ちらつかない
  - プレビューと フル は同じ `AbortController` の signal を共有。プレビューは失敗しても
    握りつぶす（フルさえ成功すれば実害はない。エラー表示はフル側の失敗でのみ行う）
  - プレビューは `PageRenderCache` に一切 `set` しない。`(pageNumber, scale)` をキーに
    しているキャッシュにプレビューのスケールが紛れ込むと、キー汚染・ピクセルバジェット
    汚染（プレビュー用の枠が本来のフル解像度ページを押し出す）につながるため
- 効果測定用に `viewer:preview-shown:<pageNumber>` / `viewer:full-shown:<pageNumber>`
  マーク（`src/perf/marks.ts`、dev 限定）を追加。前者はプレビュー表示完了まで、後者は
  マウントからフル canvas への差し替え完了まで（＝「ぼやけ解消までの総時間」）を測る
...
ページ数が多く1ページの描画が重いPDF（スキャン系・高解像度画像を含むもの）を開き、
ページ送り時に「白→ぼやけたプレビュー→フル解像度」という段階的な表示遷移になっているかを
見る。あわせて DevTools コンソールの `[perf] viewer:preview-shown:<n>: <ms>` /
`[perf] viewer:full-shown:<n>: <ms>` ログで、プレビューとフルそれぞれが出るまでの実測時間を
確認する。

### 代替案

これでもなお不十分（プレビューを挟んでもフル解像度待ちの体感が長すぎる等）であれば、次の手は
Rust 側 pdfium レンダラへの移行になる。再検討の条件は「pdfium 移行の判断」節に記載のとおり、
実機・実PDF での dev 計測（`pdfjs:first-render-page` や個々の `renderPage`）でラスタライズ
自体が支配的ボトルネックと確認された場合。
